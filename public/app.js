...
function renderWatch(main, item) {
  ...
  $('#rewindVideo', shell).onclick = () => { video.currentTime = Math.max(0, video.currentTime - 10); updateTimeline(); showControls(); };
  $('#forwardVideo', shell).onclick = () => { video.currentTime = Math.min(video.duration || Infinity, video.currentTime + 10); updateTimeline(); showControls(); };
  ...
  video.addEventListener('click', () => { if (touchRevealedControls) { touchRevealedControls = false; return; } if (shell.classList.contains('controls-hidden')) showControls(true); else togglePlayback(); });
  shell.addEventListener('touchstart', (event) => { if (event.target === video && shell.classList.contains('controls-hidden')) { touchRevealedControls = true; showControls(true); } }, { passive: true });
...
async function openSettings() {
  ...
  $$('[data-remove-folder]',dialog).forEach((b)=>b.onclick=async()=>{if(!confirm('Remove this folder and its videos from Luma? Your original files will not be deleted.'))return;await api(`/api/folders/${b.dataset.removeFolder}`,{method:'DELETE'});dialog.close();openSettings();loadLibrary()});
